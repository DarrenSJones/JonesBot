...
public class TFrinkiac {
	...
	public void process() {
		...
		MessageEmbed m1 = Frinkiac
				.process("1", "!s !d S06E04 741423\ntestencode", prefix, color, host, saved, last)
				.build();
		Assert.assertEquals(m1.getImage().getUrl(),
				"http://localhost:1080/meme/S06E04/741423.jpg?b64lines=dGVzdGVuY29kZQ==");
		...
		Assert.assertEquals(m3.getImage().getUrl(),
				"http://localhost:1080/meme/S03E22/937738.jpg?b64lines=dHJhYWFzaA==");
		...
		// Timestamp not found
		Mock.setExpectation("GET", "/api/caption?e=S06E04&t=123456", 200,
				new File(path + "timestamp_not_found.json"));
		MessageEmbed m4 = Frinkiac
				.process("1", "!s !d S06E04 123456\ntestencode", prefix, color, host, saved, last)
				.build();
		...
		Assert.assertEquals(m4.getDescription(),
				"Error parsing response, please contact your administrator.");
		Assert.assertEquals(m4.getFields().size(), 0);

		// Query not found
		Mock.setExpectation("GET", "/api/search?q=fakequery", 200,
				new File(path + "query_not_found.json"));
		MessageEmbed m5 = Frinkiac
				.process("1", "!s !d fakequery\ntestencode", prefix, color, host, saved, last)
				.build();
		...
		MessageEmbed m6 = Frinkiac
				.process("1", "!s !d Movie 999999", prefix, color, host, saved, last).build();
	...
	public void getHelp() {
		Assert.assertEquals(Frinkiac.getHelp("!"), "Blank commands will display a random image."
	...
	public void buildEmbedSaved() {
		MessageEmbed embed = Frinkiac
				.buildEmbedSaved(new Color(123, 123, 123), "host", CFrinkiacSaved.getById("1"))
				.build();
	...
	public void buildEmbedRegex() {
		MessageEmbed embed = Frinkiac
				.buildEmbedRegex(new Color(123, 123, 123), "host", CFrinkiacSaved.getById("1"))
				.build();
		Assert.assertEquals(embed.getTitle(), "Saved List: Regex");
		Assert.assertEquals(embed.getDescription(), "Contact your Admin for additions:"
				+ "\nTrash: trash" + "\nFlag: flags?" + "\nPig: (spider )?pig"
	...
	public void getResponse() {
		...
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/random"),
				TestUtils.readFile(random));
		...
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/caption?e=S00E00&t=0"),
				TestUtils.readFile(timestamp1));
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/caption?e=S99E99&t=9999999"),
				TestUtils.readFile(timestamp2));
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/caption?e=S00E00&t=1"),
				TestUtils.readFile(timestamp3));
		...
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/search?q=test1"),
				TestUtils.readFile(timestamp1));
		Assert.assertEquals(Frinkiac.getResponse(host, host + "/api/search?q=test2"),
				TestUtils.readFile(timestamp2));
	...
	public void getSaved() {
		...
		Assert.assertEquals(Frinkiac.getSaved(list, "catholic     church").name, "Catholic Church");
		Assert.assertEquals(Frinkiac.getSaved(list, "weve made a few changes").name,
				"Catholic Church");
	}
