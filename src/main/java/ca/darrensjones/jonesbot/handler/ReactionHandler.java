...
public class ReactionHandler {
	...
	public void process(GenericMessageReactionEvent event, boolean add) {
		...
		Reporter.debug(String.format("Reaction %s! Guild:[%s] Channel:[%s] MessageId:[%s] User:[%s] Emote:[%s]", action, textChannel.getGuild().getName(),
				textChannel.getName(), message.getId(), user.getName(), reactionEmote.getEmoji()), false);
	}
}